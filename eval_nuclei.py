...
def extract_features_from_json(json_data, json_name):
    ...
    for polygon_data in json_data.get('polygons', []):
        ...
        score = polygon_data.get('score', 1)  # if no probability is given, assume 1 and matching will be done by distance
        ...
        features_list.append({
            'filename': json_name,
            'category': category,
            'centroid': centroid.tolist(),  # Convert to list for easier JSON handling
            'score': score
        })
